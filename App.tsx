...
const SetGeneratorView: React.FC<{ onGenerationComplete: (images: GeneratedImage[]) => void }> = ({ onGenerationComplete }) => {
    ...
    return (
        <div className="p-8 max-w-3xl mx-auto">
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Generador de Sets de Imágenes</h1>
            <p className="text-slate-500 dark:text-slate-400 mb-8">Crea un set completo de 16 imágenes coherentes para tu proyecto web.</p>

            <div className="space-y-6 bg-white dark:bg-[#0F172A] p-8 rounded-2xl border border-slate-200 dark:border-white/10">
                <div>
                    <label htmlFor="businessType" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-2">Tipo de negocio</label>
                    <select
                        id="businessType"
                        value={config.businessType}
                        onChange={(e) => setConfig({ ...config, businessType: e.target.value as BusinessType })}
                        className="w-full p-3 bg-slate-100 dark:bg-slate-800/50 border border-slate-300 dark:border-white/10 rounded-lg text-slate-900 dark:text-slate-200 focus-ring capitalize"
                    >
                        {BUSINESS_TYPES.map(type => <option key={type} value={type}>{type}</option>)}
                    </select>
                </div>
                <div>
                    <label htmlFor="description" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-2">Descripción adicional (opcional)</label>
                    <textarea
                        id="description"
                        value={config.description}
                        onChange={(e) => setConfig({ ...config, description: e.target.value })}
                        placeholder="Ej: Estilo minimalista, tonos cálidos, ambiente de lujo..."
                        className="w-full h-24 p-3 bg-slate-100 dark:bg-slate-800/50 border border-slate-300 dark:border-white/10 rounded-lg text-slate-900 dark:text-slate-200 focus-ring"
                    />
                </div>
                 {isLoading ? (
                    <div className="space-y-3 pt-4">
                        <p className="text-center text-cyan-600 dark:text-cyan-400 font-medium">Generando {progress}/{TOTAL_IMAGES_IN_SET} — No cierres esta ventana.</p>
...
const SingleGeneratorView: React.FC<{ onImageGenerated: (image: GeneratedImage) => void }> = ({ onImageGenerated }) => {
    ...
    return (
        <div className="p-8 max-w-3xl mx-auto">
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Generador Individual (Alta Calidad)</h1>
            <p className="text-slate-500 dark:text-slate-400 mb-8">Usa Imagen 4 para crear una imagen única y de alta fidelidad a partir de un prompt.</p>
            <div className="space-y-6 bg-white dark:bg-[#0F172A] p-8 rounded-2xl border border-slate-200 dark:border-white/10">
                <div>
                    ...
                    <textarea
                        ...
                        className="w-full h-32 p-3 bg-slate-100 dark:bg-slate-800/50 border border-slate-300 dark:border-white/10 rounded-lg text-slate-900 dark:text-slate-200 focus-ring"
                    />
                </div>
                <div>
                    <label htmlFor="aspectRatio" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-2">Relación de aspecto</label>
                    <div className="grid grid-cols-5 gap-2">
                        {ASPECT_RATIOS.map(ratio => (
                            <button key={ratio} onClick={() => setAspectRatio(ratio)} className={`py-2 rounded-lg border-2 transition ${aspectRatio === ratio ? 'bg-cyan-500/20 border-cyan-500' : 'bg-slate-100 border-slate-300 hover:border-slate-400/80 dark:bg-slate-800/50 dark:border-white/10 dark:hover:border-white/20'}`}>
                                {ratio}
                            </button>
                        ))}
                    </div>
    ...
    if (images.length === 0) {
        return (
            <div className="p-8 text-center h-full flex flex-col items-center justify-center">
                <Icon name="images" className="text-7xl text-slate-400 dark:text-slate-700 mb-4" />
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Tu galería está vacía</h2>
                <p className="text-slate-500 dark:text-slate-400 mt-2">Usa uno de los generadores para empezar a crear imágenes.</p>
            </div>
        );
    }
    ...
    return (
        <div className="p-8">
            <header className="flex items-center justify-between mb-6 pb-6 border-b border-slate-200 dark:border-white/10">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Galería de Imágenes</h1>
                    <p className="text-slate-500 dark:text-slate-400">{selectedCount} de {images.length} seleccionadas</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                {images.map(image => (
                    <div key={image.id} className="relative group rounded-2xl overflow-hidden border-2 border-transparent transition-all duration-300 hover:border-cyan-500/50 hover:-translate-y-1">
                        <img src={`data:image/jpeg;base64,${image.base64}`} alt={image.name} className="w-full h-48 object-cover bg-slate-300 dark:bg-slate-800" loading="lazy" />
                        <div className={`absolute inset-0 bg-black/50 transition-opacity duration-300 ${image.selected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                            <input
                                ...
                                className="absolute top-3 left-3 h-6 w-6 rounded-md bg-slate-500/20 dark:bg-white/20 border-none text-cyan-500 focus:ring-0 cursor-pointer"
                            />
...
export default function App() {
    ...
    const [theme, setTheme] = useState<'light' | 'dark'>(() => {
        const savedTheme = localStorage.getItem('photoKo-theme');
    ...
    useEffect(() => {
        ...
        localStorage.setItem('photoKo-theme', theme);
    }, [theme]);
